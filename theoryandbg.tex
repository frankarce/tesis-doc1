...
Volume 7 , Issue 1 (January 1985), pp. 80-112] es un modelo que puede ser implementado para cualquier lenguaje que provea herramientas para crear y coordinar multiples procesos, algunas ejemplos de implementacion son CppLINDA para C++, Simple C-Linda para C o PYLinda para Python. El concepto principal de LINDA es el de Espacio de Tuplas a traves del cual los procesos se comunican. Este espacio de tuplas es una abstraccion de un espacio de memoria compartida el cual se usa de manera asociativa.
El espacio de tuplas es donde se almacena la informacion accesible para todos los procesos. Las tuplas son colecciones de campos de algun tipo soportado por el lenguaje para el que se esta implementando. Estos campos pueden ser de dos tipos, valores de un tipo soportado o variables para recibir la informacio³n obtenida del espacio de tuplas, a las variables se les añade ? por delante.
...
\item take(p) (in)
\begin{itemize}
\item p es un patron; el proceso se bloquea hasta que el espacio de tuplas le
asigna una que corresponda al patron
\item la tupla es quitada del espacio de tuplas
\end{itemize}
\item read(p) (rd)
\begin{itemize}
\item Version del "take" en que la tupla que le es asignada no se elimina del
espacio de tuplas.
\item puede haber varios read simultÃ¡neos
\end{itemize}
\end{itemize}
\begin{figure}[ht!]  
\centering  
...
\begin{enumerate}	
\item for any \textsl{a} \( \in \) \textsl{A}, [\textsl{a}] belongs to \( \tau_{A} \)
\item for any [\textsl{a}] \( \tau_{A} \), [[a]] belongs to \( \tau_{A} \)
\item if [\textit{e1}] and [\textit{e2}] belong to \( \tau_{A} \), then [\textit{e1 e2}] also belongs to  \( \tau_{A} \)  
\end{enumerate}
...
\begin{itemize}
	\item :=,=,/= 
\item [] es la tupla vacía 
\itemt 1+t2 es la concatenación 
\item si t1 es prefijo de t2, t2-t1 es la tupla resta 
\item long(t) es el número de componentes 
\item t(i), con i en el rango 1..long(t), da la componente 
\item en un patrón, NULL representa un comodín 
\item ....
\end{itemize}
\end{shaded}
\section{NoSQL}
...
%https://thenextweb.com/microsoft/2014/07/15/microsoft-starts-shipping-kinect-windows-v2-sensor-releases-free-sdk-2-0-public-preview/#.tnw_sqacrTXd
link

En el año 2010 Microsoft presentó un nuevo dispositivo que serviría como complemento o accesorio de su consola de videojuegos de sobremesa, Xbox 360. Este dispositivo que lleva el nombre de Kinect cambió muchas cosas, y no solamente en el mundo de los videojuegos. Para los gamers, por primera vez podrían controlar sus juegos en casa sencillamente con su cuerpo, o incluso con comandos de voz; pero la ciencia aprovechó este pequeño aparato para mucho más.

Sin embargo, la segunda versión de Kinect promete dejar en el pasado a su antecesor, superándolo en todo aspecto, e incorporando nuevas tecnologías que sí, Microsoft en teoría sigue dirigiendo a los videojuegos, pero también puede ser aprovechada en muchos otros aspectos, desde la investigación científica y médica, hasta la espacial, o en la edición y creación de animaciones. Las posibilidades son muy amplias.

Microsoft Research, los laboratorios más curiosos del gigante de Redmond, por así decirlo, han querido explicar a través de dos vídeos cómo funciona el nuevo Kinect de Xbox One y qué es lo que hace especial sus nuevas características técnicas.



En el caso de su sensor o su ojo, como lo llaman en Microsoft Research, cuenta con nuevas capacidades que incluyen no solo una cámara de mayor nitidez, ideal para ser utilizada en vídeoconferencias a través de Skype, por ejemplo; sino también cuenta con nuevas tecnologías que permite captar al usuario con mayor profundidad, a diferencia de la versión anterior, y en tres dimensiones. La sensibilidad del captador detalla a tal punto de percatarse si la persona abre su boca, o sonríe.

Además, cuenta con tecnología infrarroja para poder ver incluso con en completa oscuridad, sin ser afectado por luces de lámparas o móviles. Esto es peculiarmente útil para los usuarios de Xbox One que juegan o ven películas con la luz apagada, ya que podrán seguir dando órdenes a la consola usando gestos con las manos, sin ningún problema.



Pero la gran maravilla del nuevo Kinect se encuentra en la tecnología que han incorporado en su "cerebro". Su capacidad de captación va mucho más allá que simplemente ver y analizar nuestros gestos, y esto ofrece oportunidades increíbles más allá de simplemente jugar.

Kinect de Xbox One puede medir el ritmo cardíacoEl nuevo Kinect cuenta con sensores que permiten detectar no solo nuestros movimientos, sino también la posición de nuestros cuerpos, en qué sentido giramos nuestras manos, o si movemos nuestros pulgares. Además, puede captar y analizar la fuerza que ejercen nuestros músculos, y ese es quizás el mayor avance en este sentido que ofrece Microsoft con su nuevo dispositivo. Finalmente el nuevo Kinect puede medir el ritmo cardíaco de su usuario simplemente usando su cámara.

Con la primera versión de Kinect, investigadores médicos para algunos estudios, e investigadores de la NASA se plantearon la posibilidad de medir el peso de los astronautas en el espacio usando Kinect. Si todo eso era posible con la primera versión, el nuevo Kinect de Xbox One ofrecerá muchísimas más posibilidades a la ciencia y la medicina; y ni hablar de lo que podrán hacer animadores con este aparato.
